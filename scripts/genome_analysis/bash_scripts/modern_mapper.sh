...
logfilename='modern_mapping_run_'${datecode}.log

echo 'modern mapping run' $datecode > $logfilename
...
#extract unmapped reads for metagenomics
samtools view -@ $threads -b -f 4 ${samp}.sam > ${samp}_unmapped.bam
...
unmappedtotal=$(samtools view -c ${samp}_unmapped.bam)
echo "$samp $unmappedtotal" >> unmapped_reads.txt
