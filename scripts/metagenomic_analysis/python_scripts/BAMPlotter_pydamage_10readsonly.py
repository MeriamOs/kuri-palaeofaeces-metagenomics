...
parser.add_argument('-d',metavar='Misincorporation file', dest='deam', required=False, type=str, help='mapDamage2 misincorporation.txt file or new CSV (optional)')
...
if args.deam:
    ...
    # Determine format: mapDamage2 style (tab, header=1) or new CSV (comma, header=0)
    # Check columns to identify format
...
max_depth_per_sample = depth.groupby('id')['dp'].max()

plt.rcParams.update({'font.size': 11, 'figure.titlesize' : 16, 'figure.titleweight' : "bold", 'legend.fontsize': 11})
fig = plt.figure(constrained_layout=False)
gs = gridspec.GridSpec(nrows=len(filtered_head), ncols=4, figure=fig)
fig.set_figheight(((3.5*len(filtered_head))))
fig.set_figwidth(25)
    ...
    if int(read_count) > 0:
        ...
        if args.deam:
            if deam_format == 'mapDamage2':
                ...
                # The new CSV has columns CtoT-0 to CtoT-12 with proportions
                # Use positions 1-5 mapped to CtoT-0 to CtoT-4 for 5' and 3' ends both as C>T
                deamn5 = {}
                deamn3 = {}
                ...
                if not ref_row.empty:
                    for i in range(1,6):
                        ...
                        if col_name in ref_row.columns:
                            ...
                            deamn5[i] = val
#                            deamn3[i] = val  # You can comment this line out if you want to skip 3' plotting
        ...
        f.axes = plt.gca()
        f.axes.set_ylim([0, max_depth_per_sample[ID]])
        f.axes.set_xlim([0, LEN])
        ax2 = f.twinx()
        ax2.set_ylim([-1, 1])
        ax2.get_yaxis().set_ticks([])
        plt.scatter(list(MQ.keys()), list(MQ.values()), color='#DB414D', alpha=0.2)
        f.text((LEN / 100) * 0.6, max_depth_per_sample[ID] * 0.95, ID, ha='left', va='top', weight="bold")
        f.text((LEN / 100) * 0.6, max_depth_per_sample[ID] * 0.9, 'Mean Depth (MQ>=0): ' + str("{0:.2f}".format(meandepth)), ha='left', va='top')
        f.text((LEN / 100) * 0.6, max_depth_per_sample[ID] * 0.85, 'Mapped Reads (MQ>=0): ' + str(f"{int(read_count):,}"), ha='left', va='top')
        f.text((LEN / 100) * 0.6, max_depth_per_sample[ID] * 0.8, 'Mapped Reads (MQ>=' + args.mqf + '): ' + str(f"{int(read_count_mqf):,}"), ha='left', va='top')
        f.text((LEN / 100) * 0.6, max_depth_per_sample[ID] * 0.75, 'Cov% (MQ>=0): ' + str("{0:.2f}".format(per_covered)), ha='left', va='top')
        f.text((LEN / 100) * 0.6, max_depth_per_sample[ID] * 0.7, '%2X (MQ>=0): ' + str("{0:.2f}".format(perc2x)), ha='left', va='top')
        f.text((LEN / 100) * 0.6, max_depth_per_sample[ID] * 0.65, 'Mean MAPQ (>=0): ' + str("{0:.2f}".format(mean)), ha='left', va='top')
        f.axes.set_ylabel('')
        f.axes.set_xlabel('')
        legend_elements1 = [
            Line2D([0], [0], marker='o', color='#148F77', label='CovMQ>=' + args.mqf, markerfacecolor='#148F77', markersize=6),
        ...
        if args.deam and deamn5:
            ...
            legend_elements2 = [
                Line2D([0], [0], marker='o', color="#148F77", label='EDistMQ>' + args.mqf, markerfacecolor='#148F77', markersize=6),
